...
const coursewareConfigPath = `${cwd}/courseware.json`;
md5.update(name);
...
} else if (fs.existsSync(coursewareConfigPath)) {
    entry = {
        [name]: `${cwd}/courseware.json`
    };
...
const output = path.resolve(process.env.PEPPA_OUTPUT) || `${cwd}/../dist`;
